...
load("D:/obamacare_relabeled/obamacare_relabeled.RData")

obamacare_relabeled$time <- 
  as.Date(as.POSIXct(obamacare_relabeled$created_at[1:10], format = "%a %b %d %H:%M:%S %z %Y", tz = "UTC"),
          format = "%m %d")


# Creating three different dataframes by time period

d1 <- subset(d,as.Date(obamacare_relabeled$time) >= "2012-03-17" & as.Date(obamacare_relabeled$time) <= "2012-04-03")
d2 <- subset(d,as.Date(obamacare_relabeled$time) >= "2012-06-19" & as.Date(obamacare_relabeled$time) <= "2012-07-07")
d3 <- subset(d,as.Date(obamacare_relabeled$time) >= "2012-10-22" & as.Date(obamacare_relabeled$time) <= "2012-11-08")
table(obamacare_relabeled$time)


# Create three different data lists by time period
oc1 <- obamacare[as.Date(as.POSIXct(obamacare_relabeled$created_at, 
